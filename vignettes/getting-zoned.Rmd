...
Now, using a single origin zip (112) and the `ggmap` package, we can plot all of the destination zips' zones relative to that origin. 
...
bar <- 
  us %>% 
  left_join(zips_zones_lat_long, by = c("lat" = "lat_dest", "long" = "long_dest")) %>% 
  filter(origin_zip == "112") 
```
...
```{r zone_map, warning=FALSE}
# ggmap(us, extent = "panel") +
ggplot() +
  geom_polygon(data = us, aes(long, lat), fill = "white") +
  geom_polygon(data = foo,
                 # zips_zones_lat_long %>% 
                 # mutate(zone = factor(zone)) %>% 
                 # filter(origin_zip == "112"), 
             aes(long_dest, lat_dest, fill = factor(zone)),
             alpha = 0.7) +
